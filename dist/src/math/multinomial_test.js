...
var tests = function (it) {
    ...
    it('Flip a fair coin and check bounds', function (math) {
        ...
        var outcomeProbs = computeProbs(result.getValues(), 2);
    ...
    it('Flip a ten-sided coin and check bounds', function (math) {
        var numOutcomes = 10;
        var probs = ndarray_1.Array1D.zeros([numOutcomes]);
        ...
        expect(result.dtype).toBe('int32');
        expect(result.shape).toEqual([NUM_SAMPLES]);
        var outcomeProbs = computeProbs(result.getValues(), numOutcomes);
    ...
    it('Flip 3 three-sided coins, each coin is 100% biases', function (math) {
        ...
        var outcomeProbs = computeProbs(result.getValues().slice(0, NUM_SAMPLES), numOutcomes);
        test_util.expectArraysClose(outcomeProbs, [0, 0, 1], EPSILON);
        outcomeProbs = computeProbs(result.getValues().slice(NUM_SAMPLES, 2 * NUM_SAMPLES), numOutcomes);
    ...
    it('passing Array3D throws error', function (math) {
        var probs = ndarray_1.NDArray.zeros([3, 2, 2]);
