...
var ndarray_1 = require("./ndarray");
{
    var tests = function (it) {
        it('MultiRNNCell with 2 BasicLSTMCells', function (math) {
            var lstmKernel1 = ndarray_1.Array2D.new([3, 4], new Float32Array([
            ...
            var lstmBias1 = ndarray_1.Array1D.new(new Float32Array([1.090713620185852, -0.8282332420349121, 0, 1.0889357328414917]));
            var lstmKernel2 = ndarray_1.Array2D.new([2, 4], new Float32Array([
            ...
            var lstmBias2 = ndarray_1.Array1D.new(new Float32Array([0.9906240105628967, 0.6248329877853394, 0, 1.0224634408950806]));
            ...
            var c = [
                ndarray_1.Array2D.zeros([1, lstmBias1.shape[0] / 4]),
                ndarray_1.Array2D.zeros([1, lstmBias2.shape[0] / 4])
            ];
            var h = [
                ndarray_1.Array2D.zeros([1, lstmBias1.shape[0] / 4]),
                ndarray_1.Array2D.zeros([1, lstmBias2.shape[0] / 4])
            ];
            var onehot = ndarray_1.Array2D.zeros([1, 2]);
            onehot.set(1.0, 0, 0);
            var output = math.multiRNNCell([lstm1, lstm2], onehot, c, h);
            test_util.expectArraysClose(output[0][0].getValues(), new Float32Array([-0.7440074682235718]));
            test_util.expectArraysClose(output[0][1].getValues(), new Float32Array([0.7460772395133972]));
            test_util.expectArraysClose(output[1][0].getValues(), new Float32Array([-0.5802832245826721]));
            test_util.expectArraysClose(output[1][1].getValues(), new Float32Array([0.5745711922645569]));
        });
        it('basicLSTMCell with batch=2', function (math) {
            var lstmKernel = ndarray_1.Array2D.randNormal([3, 4]);
            var lstmBias = ndarray_1.Array1D.randNormal([4]);
            var forgetBias = ndarray_1.Scalar.new(1.0);
            var data = ndarray_1.Array2D.randNormal([1, 2]);
            var batchedData = math.concat2D(data, data, 0);
            var c = ndarray_1.Array2D.randNormal([1, 1]);
            var batchedC = math.concat2D(c, c, 0);
            var h = ndarray_1.Array2D.randNormal([1, 1]);
