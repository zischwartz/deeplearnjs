...
var ndarray_1 = require("./ndarray");
{
    var tests = function (it) {
        it('simple alignCorners=false', function (math) {
            ...
            var output = math.resizeBilinear3D(input, [3, 3], false);
            test_util.expectArraysClose(output.getValues(), new Float32Array([2, 2, 2, 10 / 3, 10 / 3, 10 / 3, 4, 4, 4]));
            input.dispose();
        });
        it('simple alignCorners=true', function (math) {
            var input = ndarray_1.Array3D.new([2, 2, 1], [2, 2, 4, 4]);
            var output = math.resizeBilinear3D(input, [3, 3], true);
            test_util.expectArraysClose(output.getValues(), new Float32Array([2, 2, 2, 3, 3, 3, 4, 4, 4]));
            input.dispose();
        });
        it('matches tensorflow w/ random numbers alignCorners=false', function (math) {
            ...
            var output = math.resizeBilinear3D(input, [4, 5], false);
            test_util.expectArraysClose(output.getValues(), new Float32Array([
        ...
        it('matches tensorflow w/ random numbers alignCorners=true', function (math) {
            ...
            var output = math.resizeBilinear3D(input, [4, 5], true);
            test_util.expectArraysClose(output.getValues(), new Float32Array([
