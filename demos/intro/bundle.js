...
var deeplearnjs_1 = require("../deeplearnjs");
{
    ...
    math_1.scope(function (keep, track) {
        var a = track(deeplearnjs_1.Array2D.new([2, 2], [1.0, 2.0, 3.0, 4.0]));
        var b = track(deeplearnjs_1.Array2D.new([2, 2], [0.0, 2.0, 4.0, 6.0]));
        var diff = math_1.sub(a, b);
        var squaredDiff = math_1.elementWiseMul(diff, diff);
        var sum = math_1.sum(squaredDiff);
        var size = deeplearnjs_1.Scalar.new(a.size);
        var average = math_1.divide(sum, size);
...
}
{
    ...
    var inputShape = [3];
    var inputTensor_1 = g.placeholder('input', inputShape);
    var labelShape = [1];
    var labelTensor = g.placeholder('label', labelShape);
    var multiplier = g.variable('multiplier', deeplearnjs_1.Array2D.randNormal([1, 3]));
    var outputTensor_1 = g.matmul(multiplier, inputTensor_1);
    var costTensor_1 = g.meanSquaredCost(labelTensor, outputTensor_1);
    console.log(outputTensor_1.shape);
    var learningRate = .00001;
    var batchSize_1 = 3;
    var math = new deeplearnjs_1.NDArrayMathGPU();
    var session_1 = new deeplearnjs_1.Session(g, math);
    var optimizer_1 = new deeplearnjs_1.SGDOptimizer(learningRate);
    var inputs = [
        deeplearnjs_1.Array1D.new([1.0, 2.0, 3.0]),
        deeplearnjs_1.Array1D.new([10.0, 20.0, 30.0]),
        deeplearnjs_1.Array1D.new([100.0, 200.0, 300.0])
    ];
    var labels = [
        deeplearnjs_1.Array1D.new([4.0]),
        deeplearnjs_1.Array1D.new([40.0]),
        deeplearnjs_1.Array1D.new([400.0])
    ];
    var shuffledInputProviderBuilder = new deeplearnjs_1.InCPUMemoryShuffledInputProviderBuilder([inputs, labels]);
    var _a = shuffledInputProviderBuilder.getInputProviders(), inputProvider = _a[0], labelProvider = _a[1];
    var feedEntries_1 = [
        { tensor: inputTensor_1, data: inputProvider },
        { tensor: labelTensor, data: labelProvider }
    ];
    var NUM_BATCHES = 10;
    var _loop_1 = function (i) {
        math.scope(function () {
            var cost = session_1.train(costTensor_1, feedEntries_1, batchSize_1, optimizer_1, deeplearnjs_1.CostReduction.MEAN);
    ...
    math.scope(function (keep, track) {
        var testInput = track(deeplearnjs_1.Array1D.new([0.1, 0.2, 0.3]));
        var testFeedEntries = [
            { tensor: inputTensor_1, data: testInput }
        ];
        var testOutput = session_1.eval(outputTensor_1, testFeedEntries);
        console.log('---inference output---');
        console.log('shape: ' + testOutput.shape);
        console.log('value: ' + testOutput.get(0));
    });
    inputs.forEach(function (input) { return input.dispose(); });
    labels.forEach(function (label) { return label.dispose(); });
}
